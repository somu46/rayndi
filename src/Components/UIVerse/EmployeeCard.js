...
const Card = ({ member }) => {
  return (
    <div className="w-[280px] h-[280px] bg-black rounded-[32px] p-[3px] relative shadow-[0px_70px_30px_-50px_rgba(96,75,74,0.19)] transition-all duration-500 ease-in-out group hover:rounded-tl-[55px]">
      ...
      <div className="absolute bottom-[3px] left-[3px] right-[3px] top-[80%] bg-blue-500 rounded-[29px] z-[2] shadow-inner transition-all duration-[500ms] ease-[cubic-bezier(0.645,0.045,0.355,1)] group-hover:top-[20%] group-hover:rounded-[80px_29px_29px_29px] delay-200 overflow-hidden">
        <div className="absolute bottom-0 left-6 right-6 h-[160px]">
          ...
          <span className="block text-white text-[0.9rem] mt-4">
            {member.role}
          </span>
        </div>
          ...
          <div className="flex gap-6 mx-auto">
            {member?.social?.twitter && (
              <a href={member.social.twitter} target="_blank" rel="noopener noreferrer">
                <FaTwitter className="h-5 text-white drop-shadow-[0_5px_5px_rgba(165,132,130,0.13)] hover:text-[#f55d56] hover:scale-[1.2] transition-transform duration-300" />
              </a>
            )}
